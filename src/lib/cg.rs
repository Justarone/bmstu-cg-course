...
unsafe fn find_brightnesses(normals: [Vec3d; 3], light_source: &Vec3d) -> [f64; 3] {
    [constants::ZERO_BRIGHTNESS + constants::BRIGHTNESS_RANGE * (normals[0].scalar_mul(light_source)),
     constants::ZERO_BRIGHTNESS + constants::BRIGHTNESS_RANGE * (normals[1].scalar_mul(light_source)),
     constants::ZERO_BRIGHTNESS + constants::BRIGHTNESS_RANGE * (normals[2].scalar_mul(light_source))]
}

unsafe fn divide_on_sections(int_points: [IntYPoint3d; 3], brightnesses: [f64; 3]) -> [Section; 4] {
    if int_points[0].y == int_points[2].y {
        return [Section::new(&int_points[0], &int_points[2], brightnesses[0], brightnesses[2]),
                Section::new(&int_points[2], &int_points[0], brightnesses[2], brightnesses[0]),
                Section::new(&int_points[2], &int_points[0], brightnesses[2], brightnesses[0]),
                Section::new(&int_points[0], &int_points[2], brightnesses[0], brightnesses[2])]
    };

    let midpoint2 = find_midpoint2(&int_points[0], &int_points[2], int_points[1].y);
    let midbrightness = brightnesses[0] + (brightnesses[2] - brightnesses[0]) *
        ((int_points[1].y - int_points[0].y) as f64 /
         (int_points[2].y - int_points[0].y) as f64);

    if midpoint2.x > int_points[1].x {
        [Section::new(&int_points[0], &int_points[1], brightnesses[0], brightnesses[1]),
         Section::new(&int_points[0], &midpoint2, brightnesses[0], midbrightness), 
         Section::new(&int_points[1], &int_points[2], brightnesses[1], brightnesses[2]),
